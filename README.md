...
URL: https://studentppk2026.pythonanywhere.com
База данных: SQLite (локальный файл database.db)
WSGI: Настроен для Flask
Бесплатный тариф: Без ограничений по времени работы
Особенности деплоя:
🔄 Миграция с MySQL на SQLite (для совместимости с бесплатным тарифом)
📁 Структура папок: /mysite/templates/, /mysite/static/
⚙️ Автоматическая инициализация БД при первом запуске
🔐 Безопасное хранение паролей (Werkzeug)
